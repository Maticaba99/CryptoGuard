...
import { socialAuth } from "@/lib/auth/actions";
import { ArrowRight, Loader2 } from "lucide-react";

import { fetchCsrfToken, login } from "@/store/actions/authActions";
...
export default function LoginPage() {
  ...
  const onSubmit = async (data: LoginFormValues) => {
    if (!csrfToken) {
      setServerError("CSRF token no disponible, intenta de nuevo.");
      return;
    }
    ...
    try {
      ...
      setServerError("Login fallido. Verifica tus credenciales.");
    } finally {
      setIsLoading(false);
    }
  };
